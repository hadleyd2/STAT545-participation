...
(p <- ggplot(mauna, aes(month, conc)) +
   geom_line() +
   ggtitle("Monthly CO2 Concentrations")) #Surrounded by parentheses for printing
```
...
The following code mistakenly puts the month variable on the y-axis. Fill in the `FILL_THIS_IN` so that you still obtain the same result as above.  In other words, you can override the mistake in `ggplot()` with `geom_point()`.  Note the label will still be messed up unless you add `ylab()` to the end.
...
```{r, fig.width = 5, fig.height = 2}
p +
  geom_point(colour = 'green', alpha=0.5) #Points are added to the line
```
...
```{r, fig.width = 5, fig.height = 2}
ggplot(gapminder) +
  geom_point(aes(x = gdpPercap, y = lifeExp), alpha = 0.1) #x, y are not defined in aes()
```
