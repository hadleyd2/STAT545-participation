...
```{r}
#no-intercept regression allows interpretation of the intercept term
#as average life expectancy in France for year 1952 in this case
...
```{r}
#uses the same variables as defined in my_lm (lifeExp ~ year)
predict(my_lm) %>% 
  head()
```
Or we can predict on a new dataset:
```{r}
#to use new data, we must have the same dependent variable(s) as the 
#original dataset used to create the my_lm object
years1 = data.frame(year = seq(2000, 2005))
predict(my_lm, newdata=years1)
...
ggplot(gapminder_Zimbabwe, aes(year,lifeExp)) + 
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) #se is a logical to display confidence intervals
```
...
ggplot(gapminder_Zimbabwe, aes(x=year, y=lifeExp)) + 
  geom_point() +
  geom_smooth(method="lm",
              formula = y ~ poly(I(x-1952), degree=2), data=gapminder_Zimbabwe) 
#geom_smooth() adds layer of best fit line
#note that we use the aesthetic x and y declared in ggplot() inside of geom_smooth()
